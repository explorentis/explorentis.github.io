...
function space_dust(maxcount, minX, minY, maxX, maxY, max_radius, max_mass, max_Vx, max_Vy){
    bodies = [new Planet('Солнце', 0, 0, 695.8, 1988920, 0, 0, 'Yellow', 10, 'Red', 'звезда'),
        new Planet('Меркурий', 46000, 0, 4.879 / 2, 0.33, 0, calc_perihelion_speed(1988920, 0.205, 69800), 'Brown', 0, 'Black', 'планета'),
        new Planet('Венера', -107500, 0, 12.104 / 2, 4.87, 0, -calc_perihelion_speed(1988920, 0.007, 108900), 'Cornsilk', 0, 'Brown', 'планета'),
        new Planet('Земля', 0, 147100, 12.756 / 2, 5.97, calc_perihelion_speed(1988920, 0.017, 152100), 0, 'Blue', 0.1, 'Cyan', 'планета'),
        new Planet('Луна', 0, 147100 + 363, 3.475 / 2, 0.073, calc_perihelion_speed(1988920, 0.017, 152100) + calc_perihelion_speed(5.97, 0.055, 406), 0, 'Yellow', 0, 'Grey', 'спутник'),
    ...
    max_obj = getRandomInt(1, maxcount);
    for (var i = 0; i != max_obj; i++){
        bodies[bodies.length] = new Planet('Ast-' + i, getRandomArbitary(minX, maxX), getRandomArbitary(minY, maxY), getRandomArbitary(0, max_radius), getRandomArbitary(0, max_mass), getRandomArbitary(-max_Vx, max_Vx), getRandomArbitary(-max_Vy, max_Vy), 'Grey', 0, 'Grey')
    }
    solsystem = new SolarSystem(bodies);
    solsystem.zoom(0.01);
    solsystem.draw();
    document.getElementById('mainmenu').parentNode.removeChild(document.getElementById('mainmenu'));
}